...
function HomePage() {
  ...
  const uniqueRoomTypes = [
    ...new Set(
      rooms.map((room) => {
        const normalized = room.roomType.toLowerCase().trim();
  ...
  const handleRateUs = () => {
    ...
    if (!token) {
      const proceed = window.confirm(
        "You need to log in to rate us. Do you want to proceed to the login page?"
      );
      if (proceed) {
        navigate("/Login");
      }
  ...
  const applyFilters = () => {
    ...
    if (locationFilter) {
      filtered = filtered.filter(room => room.roomCity === locationFilter);
    }
    
    if (propertyTypeFilter) {
      filtered = filtered.filter(room => 
        room.roomType.toLowerCase().trim() === propertyTypeFilter.toLowerCase().trim()
      );
    }
    
    if (priceFilter === 4000) {
      filtered = filtered.filter(room => room.price < 10000);
    } else if (priceFilter === 12000) {
      filtered = filtered.filter(room => room.price >= 10000 && room.price <= 15000);
    } else if (priceFilter === 20000) {
      filtered = filtered.filter(room => room.price > 15000);
    }
        ...
        <div className="container">
          ...
          <div className="stats-right">
            ...
            <div className="buttons">
              <button className="primary-button" onClick={handleRateUs}>Rate Us</button>
            </div>
          </div>
        </div>
      </section>
